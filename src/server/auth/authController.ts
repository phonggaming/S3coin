...
export async function registerHandler(req: Request, res: Response): Promise<void> {
  ...
  const newUser = queryOne<AuthUser>('SELECT id, username, email, miner_token, balance_s3, total_mined, created_at, last_login, status, is_admin FROM users WHERE username = ?', [cleanUsername]);
